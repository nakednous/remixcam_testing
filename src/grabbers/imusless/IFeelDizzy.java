...
public class IFeelDizzy extends PApplet {
	...
	public void setup() {
		...
		scene.setVisualHints(Constants.PATHS);
	...
	public void keyPressed() {
		if (key == 'p') {
			println("M1:");
			message1.position().print();
			message1.orientation().print();
			message1.magnitude().print();

			println("M2:");
			message2.position().print();
			message2.orientation().print();
			message2.magnitude().print();

			println("Image:");
			image.position().print();
			image.orientation().print();
			image.magnitude().print();

			GeomFrame kfi;
			int keys = scene.view().keyFrameInterpolator(1)
					.numberOfKeyFrames();
			for (int i = 0; i < keys; i++) {
				kfi = scene.view().keyFrameInterpolator(1).keyFrame(i);
				println("key " + i);
				kfi.position().print();
				kfi.orientation().print();
				kfi.magnitude().print();
			}
		}
	}
