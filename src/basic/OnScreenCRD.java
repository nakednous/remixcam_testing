...
public class OnScreenCRD extends PApplet {
	...
	public void draw() {
		...
		stroke(255,255,0);
		Vec p1 = scene.center();
		Vec p2 = Vec.add(scene.center(), Vec.mult(scene.camera().upVector(), 230 * scene.camera().pixelSceneRatio(scene.center())));	  
	...
	public void parameteriseNavigator() {
		...
		this.boxLenghtRatio = boxLenght	* scene.camera().pixelSceneRatio(iFrame.position());
	}	
